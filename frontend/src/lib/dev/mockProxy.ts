  ...
  import.meta.hot.on('mock:commands-list', ({ commands }) => {
    ...
    commands.forEach((cmd: { name: string, description: string }) => {
      ...
      if (clientCommands[commandName as keyof typeof clientCommands]) {
        mocksCtrl[commandName] = clientCommands[commandName as keyof typeof clientCommands];
      } else {
        mocksCtrl[commandName] = (...args: any[]) => sendToServer(commandName, ...args);
      }
    ...
    const availableCommands = Object.keys(mocksCtrl).sort().map(cmd => `  - mocksCtrl.${cmd}()`).join('\n');
    console.log(
