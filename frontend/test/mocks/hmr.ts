...
export function initializeHmrBridge(server: ViteDevServer, io: SocketIOServer, printer: Printer) {
  ...
  try {
    const commandsJsonPath = path.resolve(process.cwd(), 'test/mocks/mockCommands.json');
    ...
    console.warn('⚠️ [HMR Bridge] Could not load mock commands manifest. Falling back to live parsing.');
    try {
      commandsWithDocs = parseMockCommandsFromSources();
      console.log('✅ [HMR Bridge] Mock commands parsed live successfully.');
    } catch (parseError) {
      console.error('🔥 [HMR Bridge] CRITICAL: Failed to load and parse mock commands.', parseError);
    }
  }
