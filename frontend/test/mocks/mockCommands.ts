...
export const mockCommands: { [key: string]: (ctx: MockCommandContext, ...args: any[]) => void } = {
  ...
  online: ({ io, printer }) => {
    ...
    if (!(io as any).server) {
      console.log('📠 [Kobra Mock] Socket.IO server is closed. Restarting Vite to recover.');
      // Touches the vite config file to trigger a server restart.
      const now = new Date();
      try {
        fs.utimesSync('./vite.config.ts', now, now);
      } catch (err) {
        console.error('🔥 [Kobra Mock] Failed to touch vite.config.ts to trigger restart:', err);
      }
      return;
    }
    printer.online = true;
